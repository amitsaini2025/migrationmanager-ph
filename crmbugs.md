...
**Method:** Area-by-area static code review (controllers, routes, services, views, JS). Bugs only — no fixes applied.  
...
### A1. Critical — CRM “Remember Me” stores plaintext password in a cookie
**What breaks:** Checking Remember Me writes the raw staff password into a cookie; the login form re-fills it from that cookie. Anyone with cookie access (XSS, shared PC, browser sync) obtains the password. Laravel’s session remember-token already exists; this custom cookie is unsafe and redundant.  
**Evidence:** `app/Http/Controllers/Auth/AdminLoginController.php` ~120–126; `resources/views/auth/admin-login.blade.php` ~40 (`Cookie::get('password')`).  
**Reproduce:** Log in with Remember Me → DevTools Cookies → `password` holds plaintext → reopen `/login` → password field prefilled.

### A2. Critical — Staff `/api/admin-login` writes `staff.id` into `refresh_tokens.user_id` (FK → `admins`)
**What breaks:** After the dedicated `staff` table, `adminLogin` authenticates `Staff` but inserts `user_id = $staff->id` into `refresh_tokens`, which foreign-keys to `admins.id`. New staff IDs are generally not valid `admins` rows → login fails with a DB FK error. If an ID collides with a client in `admins`, insert can succeed and `/api/refresh` loads that **client** via `Admin::find()`, mixing identities. Same pattern risk for `device_tokens`.  
...
**Reproduce:** `POST /api/admin-login` with valid staff credentials (roles 1/12/13/16). Expect 500 on refresh-token insert, or wrong identity on refresh if IDs collide.
...
**Evidence:** `app/Http/Controllers/API/ClientPortalController.php` ~45–48 (login), ~572 (refresh), ~353–357 (reset).  
...
**What breaks:** `AdminLoginController` uses `Auth::guard('admin')->attempt()` with no `status` / archive check. Disabled staff (`status = 0`) can still authenticate if credentials are valid.  
...
### B1. High — Grant expiry uses UTC in PHP but `NOW()` in SQL list filters
**What breaks:** Grants are written/checked with `Carbon::now('UTC')`, while list/document/booking restrictions use `ends_at > NOW()`. App timezone is `Australia/Melbourne`. If DB `NOW()` is local, active grants can disappear from lists while `hasActiveGrant()` / detail access still treat them as valid (or the reverse) — inconsistent visibility for the grant window.  
**Evidence:** `app/Services/CrmAccess/CrmAccessService.php` ~88–96; `app/Support/StaffClientVisibility.php` ~322–325; `config/app.php` timezone `Australia/Melbourne`.  
**Reproduce:** Non-exempt staff takes quick access → client detail works → reload restricted client list; grant may be excluded from list SQL while detail still allows access.

### B2. Medium — Cross-access grants can be created for super-admin-only locked clients
**What breaks:** `AccessGrantController::quick` / `supervisor` do not block super-admin-only locked file IDs. A grant is created, but `canAccessClientOrLead` still denies non–role-1 users because the locked check runs before grant checks. Staff get success then still cannot open the record.  
**Evidence:** `AccessGrantController` (no locked-client check on create) vs `StaffClientVisibility::canAccessClientOrLead` ~551–553.  
...
**What breaks:** Route posts to `/clients/edit` with no `{id}`; both GET and POST hit `edit($id)`. On POST, `$id` is null → redirect unauthorized. Method only renders the edit view; it never persists. Forms still `action="{{ route('clients.update') }}"`. Section AJAX saves work; native submit (Enter in a field, etc.) fails and saves nothing.  
...
**What breaks:** `convertToClient` ignores request body/IDs, loads `Lead::withArchived()->paginate(500)`, and converts every row. Super-admin only, but a single GET is a destructive mass conversion. Route still registered.  
...
**What breaks:** Workflows id=2 (Australian Education) and id=3 (General) exist. Stages commonly have `workflow_id=3` but stale `w_id=2`. Modern stage code filters by `workflow_id`; matters on workflow 2 get no next/prev stage. Legacy `w_id` paths still see stages under the wrong key.  
**Evidence:** `ClientPortalController` modern queries ~3724–3728; legacy `w_id` usage ~3599–3601, ~3639–3641, ~4572–4574.  
...
**Evidence:** `PublicDocumentController.php` ~705–720 vs `ClientPortalController::updateClientMatterNextStage` ~3739–3750.  
...
**Evidence:** `ClientPortalController.php` ~3680–3784, ~3913–3970, ~5076+.  
...
**Evidence:** `routes/documents.php` ~171–172; `PublicDocumentController::getPage` ~782–792; `resources/views/documents/sign.blade.php` ~410.  
...
**Evidence:** `routes/documents.php` ~178–182 vs ~244–248; `PublicDocumentController.php` ~1029; `resources/views/documents/index.blade.php` ~56.  
...
**What breaks:** `ClientPortalBillingController::updateInvoice` trusts client-supplied `payment_status: "completed"` and an opaque `payment_token`, then calls `markFullyPaidFromClientPortal` (zeros balances / sets `invoice_status = 1`) with no Stripe (or other gateway) verification. Authenticated clients can zero portal invoices.  
**Evidence:** `app/Http/Controllers/API/ClientPortalBillingController.php` ~144–206; `app/Services/InvoicePaymentSyncService.php` ~249–304; route `POST /api/billing/invoice-update`.  
**Reproduce:** Auth as portal client → POST `payment_status: completed` + any token for a sent unpaid invoice.

### H2. High — Void invoice can reverse the wrong fee transfers
**What breaks:** If Method 1 (by `invoice_no`) finds nothing, Method 2 matches fee transfers by `withdraw_amount = $invoiceAmount` and allows `invoice_no` null/empty. `$invoiceAmount` is computed as **max of line amounts**, not the invoice sum, so multi-line invoices often miss the real transfer and/or match unrelated same-amount transfers.  
**Evidence:** `ClientAccountsController::void_invoice` ~4122–4224.
...
**What breaks:** Client-fund, office, and other receipt paths do `orderBy receipt_id desc → +1` with no `GET_LOCK` (unlike invoice numbers). Concurrent saves can collide on `receipt_id`.  
**Evidence:** `ClientAccountsController.php` ~323–324, ~1888–1889, ~2724–2725 (contrast locked `createInvoiceNumber` ~1009–1034).
...
**What breaks:** `recordAppointmentPaymentWallet` and `recordAppointmentPaymentWithoutLoginWallet` accept any `payment_intent_id` string and immediately write `status = succeeded` / `payment_status = completed` with no `PaymentIntent::retrieve`, amount check, or status check. The public without-login wallet route is unauthenticated.  
**Evidence:** `ClientPortalAppointmentController.php` ~2551–2580, ~2865–2894; `routes/api.php` ~68, ~271.  
...
**What breaks:** Public `POST /api/payments/create-payment-intent` creates Stripe PaymentIntents from caller-controlled `amount` (min 50 cents), defaults currency to **USD** (not AUD), and returns `client_secret`. No appointment binding, auth, or rate limit. Enables charge creation / abuse of the Stripe account.  
**Evidence:** `routes/api.php` ~70–144.

### I3. High — Public record-payment weakly binds PaymentIntent to appointment
**What breaks:** `recordAppointmentPaymentWithoutLogin` only needs `appointment_id` + any succeeded PI whose amount matches. Metadata `appointment_id` is checked only if present. A PI created via the public create-intent endpoint (or any matching-amount PI) can be attached to another unpaid appointment.  
**Evidence:** `StripePaymentService::recordPaymentByIntent` ~520–527; `ClientPortalAppointmentController::recordAppointmentPaymentWithoutLogin` ~2684–2728; `routes/api.php` ~67.

### I4. Medium — `processPayment` rolls back then tries to update the payment row
**What breaks:** On card/API errors, `DB::rollBack()` undoes `AppointmentPayment::create`, then `$payment->update([... 'failed' ...])` runs against a non-existent row — failure audit trail is lost.  
**Evidence:** `StripePaymentService.php` ~140–153, ~181–192, ~256–265.

### I5. Medium — PaymentIntent amount uses truncating cast (cent rounding bug)
**What breaks:** `createPaymentIntent` uses `(int) ($amount * 100)` while public/record paths use `round()`. Floating point (e..g. `19.99 * 100`) can undercharge by 1 cent vs CRM amount checks.  
**Evidence:** `StripePaymentService.php` ~336 vs ~426, ~506.
...
**What breaks:** After Stripe recording succeeds, code only `Log::info('... should sync with Bansal API')` inside a try/catch that never calls `syncStatus` / `updateAppointmentStatus`. CRM shows paid; website can stay unpaid. Same pattern on auth and public record-payment paths. Public pay-by-link also never syncs.  
...
**What breaks:** Public webhook routes update `SmsLog` from request body with no Twilio signature / Cellcast auth check. Attackers can forge delivery status.  
...
**What breaks:** `ClientEoiRoiController` disables AdminPolicy auth and explicitly comments out `client_id` ownership checks on show/update/delete/reveal-password/verify/send-email. Any authenticated staff can load, mutate, delete, or decrypt EOI passwords for another client’s EOI by ID under a different `{client}` URL.  
...
**What breaks:** Client can POST any `client_matter_id`; code loads the matter and messages that matter’s staff with no `client_matters.client_id === auth id` check. Cross-matter spam / leaking presence into other matters’ threads.  
...
### N2. High — API broadcasting auth omits `client_id`; staff/client ID collision
**What breaks:** `/api/broadcasting/auth` authorizes `private-matter.{id}` only via `sel_migration_agent|responsible|assisting` (staff FKs), not `client_id`. So (1) legitimate clients cannot subscribe to their matter channels; (2) after staff/admin ID overlap, a client whose `admins.id` equals a staff assignee ID on another matter can join that private channel.  
**Evidence:** `routes/api.php` ~349–371; contrast `routes/channels.php` ~46–55 (includes `client_id`); overlap noted in `docs/PLAN_DEDICATED_STAFF_TABLE.md`.
...
**What breaks:** Views live under `resources/views/crm/forms/*`, but controller returns `forms.index` / `forms.show` / `forms.create`. `destroy` redirects to `route('forms.index')` which is not registered (only store/show/edit/update/destroy/preview/pdf). Show/destroy/index break with View/RouteNotFound.  
**Evidence:** `Form956Controller.php` ~33–37, ~146–150, ~1100–1105; views at `resources/views/crm/forms/`; routes in `routes/clients.php` ~378–384.
...
**What breaks:** Without `admin_id`, command runs `Admin::where('status', 1)->get()` — post staff-table migration that is essentially all active clients/leads, not staff. Wrong tokens / huge blast radius.  
...
### Q1. Medium — `unix://` PYTHON_CONVERTER_URL is non-functional
**What breaks:** `isUnixSocket()` is detected but HTTP client still does normal `Http::get/post($this->apiUrl . '/health')`, which cannot talk to a Unix socket. Health checks and conversions fail when that URL scheme is configured (as in some deploy docs).  
...
1. Payment trust bugs (H1, I1, I2, I3) — free mark-paid / open PaymentIntent  
