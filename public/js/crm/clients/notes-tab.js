/**
 * Client Detail — Notes (noteterm) tab lazy fragment loader.
 * Stub on first paint; load once on click; re-bind pin/type pills/matter filter.
 * Personal Details, Activity feed, and sidebar/matter context stay eager (not lazy).
 */
(function() {
    'use strict';
    ...
    function notesTabFragmentUrl(tabEl) {
        ...
        var url = '/clients/detail-noteterm-tab/' + encodeURIComponent(encodeId);
        if (matterRef) {
    ...
    function applyNotesMatterFilter() {
        if (typeof window.filterNotes === 'function') {
            window.filterNotes();
            return;
        }
        if (window.SidebarTabs && typeof window.SidebarTabs.filterNotesByMatter === 'function') {
            var matterId = '';
            if (window.jQuery) {
                matterId = window.jQuery('#sel_matter_id_client_detail').val() || '';
            }
            window.SidebarTabs.filterNotesByMatter(matterId);
        }
    }
    ...
    function needsFragmentLoad(tabEl, force) {
        ...
        return tabEl.getAttribute('data-noteterm-lazy') === '1'
            || !!tabEl.querySelector('[data-noteterm-lazy-placeholder]');
    }
    ...
    function ensureNotesTabLoaded(force) {
        ...
        if (!needsFragmentLoad(currentTab, force)) {
            initNotesTabAfterInject(currentTab);
        ...
        .then(function(html) {
            ...
            return activateInjectedScripts(newTab).then(function() {
                initNotesTabAfterInject(newTab);
                return newTab;
            });
        })
        .catch(function(err) {
    ...
    function bootNotesTabIfNeeded() {
        ...
        ensureNotesTabLoaded().catch(function(err) {
            console.error('[Notes] Boot load failed', err);
        });
    }
